...
# Compila os Lambdas Go do domínio Audit para Lambda arm64 (Graviton).
set -euo pipefail
cd "$(dirname "$0")/src"
...
go mod tidy
mkdir -p ../dist
for fn in audit-writer audit-api audit-archiver; do
  echo "building $fn"
  go build -tags lambda.norpc -trimpath -ldflags="-s -w" -o "bootstrap" "./cmd/$fn"
  zip -j "../dist/$fn.zip" bootstrap >/dev/null
  rm -f bootstrap
done
echo "ok: dist/{audit-writer,audit-api,audit-archiver}.zip"
